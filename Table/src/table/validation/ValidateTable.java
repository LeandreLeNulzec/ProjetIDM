...
/**
 * Réalise la validation de modèles conformes à Table à l'aide du validateur et
 * affiche le résultat.
 * 
...
public class ValidateTable {
	...
	private static void afficherResultat(ValidationResult resultat) {
		...
		afficherErreurs("- Colonne", resultat.getRecordedErrorsFor(TablePackage.COLONNE));
		afficherErreurs("- ColonneProvient", resultat.getRecordedErrorsFor(TablePackage.COLONNE_PROVIENT));
		afficherErreurs("- ColonneDerivee", resultat.getRecordedErrorsFor(TablePackage.COLONNE_DERIVEE));
	}
